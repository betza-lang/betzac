    ...
    describe "discover" $ do
        it "reports a system failure when the workspace root doesn't exist" $
            withSystemTempDirectory "betzac-driver-spec" $ \dir -> do
                ...
                result <- Driver.discover TIO.readFile Nothing (dir </> "nope") (dir </> "f0.betza") Nothing (optionsFromFlags [])
                case result of
        ...
        it "reports a system failure when the target file doesn't exist" $
            withSystemTempDirectory "betzac-driver-spec" $ \dir -> do
                result <- Driver.discover TIO.readFile Nothing dir (dir </> "nope.betza") Nothing (optionsFromFlags [])
                case result of
                    Left problem -> causeOf (semKind problem) `shouldBe` "system"
        ...
        it "reports using unknown for an unresolvable using target, without aborting discovery" $
            withSystemTempDirectory "betzac-driver-spec" $ \dir -> do
                ...
                result <- Driver.discover TIO.readFile Nothing dir (dir </> "f0.betza") Nothing (optionsFromFlags [])
                case result of
    ...
    describe "using directives" $ do
        let withDep body =
            ...
            resolve dir main = do
                ...
                result <- Driver.discover TIO.readFile Nothing dir (dir </> "main.betza") Nothing (optionsFromFlags [])
                case result of
                    Left _ -> fail "did not expect a system failure"
        ...
        it "discovers every file in an acyclic chain exactly once, with no errors" $
            hedgehog $ do
                ...
                result <- liftIO $ withSystemTempDirectory "betzac-driver-spec" $ \dir -> do
                    writeChain dir n
                    Driver.discover TIO.readFile Nothing dir (dir </> "f0.betza") Nothing (optionsFromFlags [])
                case result of
...
resolvedChain dir = Driver.discover TIO.readFile Nothing dir (dir </> "f0.betza") Nothing (optionsFromFlags [])
